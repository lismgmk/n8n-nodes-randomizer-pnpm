...
export class RandomizerNodePnpm implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'RandomizerPnpm',
		name: 'randomizerNodePnpm',
		...
		properties: [
			...
			},
			{
				...
				default: 60,
				description:
					'If you set the percentage to 60, then the node will select the A input 60% of the time and the B input 40% of the time',
			},
		],
	};
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][] | null> {
		...
		if (option === 'random') {
			...
			if (!inputDataA || !inputDataB) {
				...
				throw new Error(
					'No input data found. Make sure to connect two inputs and that they contain data.',
				);
			}
